...
namespace OneStore.Purchasing
{
    public class PurchaseClientImpl : IPurchaseExtensions
    {
        ...
        public void Initialize(IPurchaseCallback callback)
        {
            ...
            _purchaseUpdatedListener.OnPurchasesUpdated += ParsePurchaseUpdatedResult;
        ...
        public void QueryProductDetails(ReadOnlyCollection<string> productIds, ProductType type)
        {
            ...
            ExecuteService(() => {
                ...
                productDetailsListener.OnProductDetailsResponse += ParseProductDetailsResult;
        ...
        public void QueryPurchases(ProductType type)
        {
            ...
            ExecuteService(() => {
                ...
                queryPurchasesListener.OnPurchasesResponse += ParseQueryPurchasesResult;
                _purchaseClient.Call(
        ...
        private void ParseProductDetailsResult(ProductType type, AndroidJavaObject javaIapResult, AndroidJavaObject javaProductDetailList)
        {
            var iapResult = _iapHelper.ParseJavaIapResult(javaIapResult);
            if (!iapResult.IsSuccessful())
            {
        ...
        private void ParsePurchaseUpdatedResult(AndroidJavaObject javaIapResult, AndroidJavaObject javaPurchasesList)
        {
            _productInPurchaseFlow = null;
            var iapResult = _iapHelper.ParseJavaIapResult(javaIapResult);
            if (!iapResult.IsSuccessful())
        ...
        private void ParseQueryPurchasesResult(ProductType type, AndroidJavaObject javaIapResult, AndroidJavaObject javaPurchasesList)
        {
            _productInPurchaseFlow = null;
            var iapResult = _iapHelper.ParseJavaIapResult(javaIapResult);
            if (!iapResult.IsSuccessful())
        ...
        private void QueryPurchasesInternal(string productId, ProductType type, Action<IapResult, PurchaseData> callback)
        {
            ...
            queryPurchasesListener.OnPurchasesResponse += (_, javaIapResult, javaPurchasesList) => {
                ...
                if (!iapResult.IsSuccessful())
                {
                    ...
                    HandleErrorCode(iapResult, () => {
                        RunOnMainThread(() => callback?.Invoke(iapResult, null));
                    });
                    return;
                }
